...
async function getsongs(folder) {
    ...
    for (const song of songs) {
        songUL.innerHTML = songUL.innerHTML + `<li><img class="invert" width="34" src="Pics/music.svg" alt="">
                            <div class="info">
                            ...
                            <div class="playnow">
                                <span>Play Now</span>
                                <img id="libplay" class="invert pblb" src="Pics/play.svg" alt="">
                            </div> </li>`;
    }
            ...
            cardContainer.innerHTML = cardContainer.innerHTML + `<div data-folder="${folder}" class="card">
                        <div class="play">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="100" height="100">
                                <circle cx="50" cy="50" r="25" fill="#1fdf64" stroke="#1fdf64" stroke-width="2"/>
                                <g clip-path="url(#clip)">
                                <svg x="31" y="29" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="40" height="40">
...
async function main() {
    ...
    previous.addEventListener("click", () => {
        let index = songs.indexOf(currentsong.src.split(`${f}/`)[1])
        console.log(index);
        if ((index - 1) >= 0) {
    ...
    // event listener for volume mute
    document.querySelector(".volume > img").addEventListener("click", e => {
        if (e.target.src.includes("Pics/volume.svg")) {
            e.target.src = e.target.src.replace("Pics/volume.svg", "Pics/mute.svg")
            currentsong.volume = 0;
            // document.getElementsByTagName("input")[0].value=0;
        }
        else {
            e.target.src = e.target.src.replace("Pics/mute.svg", "Pics/volume.svg")
